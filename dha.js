...
petik = '```'
fake = 'CREATOR BOT\KGSTORE'//GANTI NAMA KAMU BEP
ban =[]
...
module.exports = dha = async (dha, mek) => {
	try {
        ...
        const freply = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { "contactMessage": { "displayName": `${pushname}`, "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:XL;${pushname},;;;\nFN:${pushname},\nitem1.TEL;waid=${senderr.split('@')[0]}:${senderr.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, "jpegThumbnail":fs.readFileSync('./media/KGSTORE.jpg')
        }}}
       const math = (teks) => {
      ...
      const hideTag = async function(from, text){
	       ...
	       dha.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/KGSTORE.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}  
...
dha.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
        ...
        case 'menu':
        case 'help':
        menu =`Hai Kak.....\n*${pushname}*\n\`\`\`Saya KGSTORE-BOT,SENANG BISA BERTEMU DENGANMU HARI INI\`\`\`
...
©𝑪𝒓𝒆𝒂𝒕𝒐𝒓 ©kurrtod`
               ...
               imageMsg = (await dha.prepareMessageMedia(fs.readFileSync(`./media/KGSTORE.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/KGSTORE.jpg`)})).imageMessage

               buttonsMessage = {
               contentText: `${menu}`,
               footerText: '\`\`\`MAU BELI SC INI HUBUNGIN KGSTORE WA 6285875085575\`\`\`', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
       ...
       case 'store':
               ...
               const yy = {title: 'List Button Menu' + nombor++,
                    rows: [
                       {
                        title: `${listmenuu[startnum++]}`,
                        description: `\n\n\n\n\`\`\`JAN LUPA GUNAKAN JASA ADMIN KGSTORE\`\`\``,
                        rowId: `${prefix}${x}`
                      }
                    ]
                   }
               ...
               listmsg(from, `${ucapanWaktu}`,   `Hai kak.....\n*${pushname}*\nJangan Lupa Gunakan Jasa Admin Yahh`, list)
               break
//------------------< Game >------------------- 
        case 'limitgame': 
        case 'balance': 
...
//------------------< Premium >-------------------
       ...
       case 'buypremium':
       case 'sewabot':
              gopeynya = 'https://i.ibb.co/XD3VGPZ/0fef4e7f6f05.jpg'
              teksnya = `*── 「 PRICE LIST 」 ──*
...
//------------------< bayar menu >-------------------  
case 'bayar':
gopeynya = 'https://i.ibb.co/8bzTBZg/294eadbd187d.jpg'
teksnya = ` *「PAYMENT」*
• GOPAY : scan diatas
• DANA : 085875085575
• OVO  : 085875085575
...
case 'tiktok':
              ...
              imageMsg = (await dha.prepareMessageMedia(fs.readFileSync(`./media/KGSTORE.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/KGSTORE.jpg`)})).imageMessage
              buttonsMessage = {footerText:'Jangan Lupa Subscribe Yt KGSTORE-BOT\n Helpme tu 1k subscriber', imageMessage: imageMsg,
...
dha.sendMessage(from, tapib1, document, { quoted: mek, filename:`KGSTORE-BOT ~ 404 ${vipi}.mp3`, mimetype: 'audio/application' })
...
"groupName": `KGSTORE-BOT ~ 404 ${vipi}`,
...
case 'getvn':
      ...
      case 'broadcast':
             ...
             for (let _ of anu) {
             sendMess(_.jid, `*「 PESAN SIARAN KGSTORE-BOT」*\n\n${body.slice(4)}`)
}
             reply('Suksess broadcast')
}
             break
...
//------------------< Lainnya >-------------------
        ...
        case 'info':  // Jangan Di Ubah Plise
               urlinfo = 'https://i.ibb.co/XD3VGPZ/0fef4e7f6f05.jpg'
...
case 'tahta':
         ...
         break    
case 'pornhub':
         if (args.length == 0) return reply(`Contoh: ${prefix + command} KGSTORE-BOT`)
         txt1 = args[0]
         txt2 = args[1]
